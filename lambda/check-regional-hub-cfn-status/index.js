...
/**
 * CloudFormation Status Check Lambda - Checks stack status in target region
 * Called by the Regional Hub Creation/Deletion State Machines
 */
exports.handler = async (event) => {
  console.log('CheckRegionalHubCfnStatus event:', JSON.stringify(event, null, 2));
  ...
  try {
    ...
    if (stack.StackStatus === 'CREATE_COMPLETE' && stack.Outputs) {
      ...
      // Parse PrivateSubnetIds from comma-separated string to array
      if (outputs.PrivateSubnetIds) {
        outputs.PrivateSubnetIdsArray = outputs.PrivateSubnetIds.split(',');
      }
    }
