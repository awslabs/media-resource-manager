...
exports.handler = async (event) => {
  console.log('Checking auto-login config status:', JSON.stringify(event, null, 2));
  ...
  try {
    ...
    console.log('Auto-login config command status:', status);
    
    if (status === 'Success') {
      console.log('Auto-login configuration completed successfully');
      ...
      console.log('Auto-login configuration still in progress');
      return {
        ...event,
      ...
      console.log('Auto-login configuration failed with status:', status);
      console.log('Output:', result.StandardOutputContent);
      ...
      return {
        ...
        autoLoginInProgress: false,
        autoLoginStatus: status,
        error: 'Auto-login configuration failed: ' + status
      };
    }
  } catch (error) {
    console.error('Error checking auto-login config status:', error);
