...
def invoke_regional_lambda(region, event, context):
    ...
    regional_function_name = f'arn:aws:lambda:{region}:{context.invoked_function_arn.split(":")[4]}:function:{acronym}-regional-dcv-readiness-check-linux'
...
def lambda_handler(event, context):
    ...
    print(f"Checking DCV readiness for Linux instance: {instance_id} (region: {region or 'primary'})")
    ...
    try:
        ...
        for server in servers_data.get('Servers', []):
            ...
            if ec2_id == instance_id:
                ...
                is_ready = server_id is not None
