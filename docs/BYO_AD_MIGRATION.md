...
1. **A reachable Active Directory.** From the MRM VPC's private subnets, TCP/UDP 389 (LDAP), TCP 636 (LDAPS, if used), and TCP/UDP 53 (DNS) must reach your domain controllers. If your AD is on-prem, this typically means Direct Connect or Site-to-Site VPN into the MRM VPC. If your AD is in another AWS account, a VPC peering or Transit Gateway connection.
